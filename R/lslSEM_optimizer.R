      ...
      if (merror_idc[p] == T) {
        ...
        for (j in idx[idx > p]) {
          if (all(merror_idc)) {
            ...
            merror_idcp <- merror_idc
            merror_idcp[p] <- F
            Psivp <- matrix(0, P-1, P-1)
            Psivp[merror_idc[-p], merror_idc[-p]] <- solve(Ps[merror_idcp, merror_idcp])          
          }
        ...
        Ps[p, p] <- Cep[p, p] - 2*Usum + Vsum + matrix(Ps[p, -p], 1, (P - 1)) %*% Psivp %*% matrix(Ps[-p, p], (P - 1), 1)
      } 
    }    
  }
  return(Ps)
...
.sem_ecm <- function(pattern, value, penalty_fit, control, info, obs_moment) {
  N <- info$N
  P <- info$P
  M <- info$M
  Qall <- info$Qall
  ...
  if (all(!.is_est(Psp))) {
    Psp_type = 1
  } else if (all(diag(.is_one(Psp))) & all(!.is_est(Psp[lower.tri(Psp)]))) {
    Psp_type = 2
  ...
  if (all(!.is_est(Php))) {
    Php_type = 1
  } else if (all(diag(.is_one(Php))) & all(!.is_est(Php[lower.tri(Php)]))) {
    Php_type = 2
  ...
  for (it in 1:maxit) {
    ...
    dpl_new <- dml + rpl 
    if (abs(dpl - dpl_new) < epsilon) break
    dpl <- dpl_new
  }
  Q <- sum(theta[is.na(thetap)] != 0, na.rm = T) + sum(thetap, na.rm = T)
  df <- P * (P + 3) / 2 - Q
